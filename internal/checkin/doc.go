...
// parses the wire payload and resolves it onto a travelmap account first, and
// [Sync] for one fetch run, which pages a linked account's check-ins through
// internal/foursquare and writes each one the same way — so that however many
// collection paths feed it, they agree on how a duplicate is recognised and
// on which fields a repeat write overwrites. See "checkins" in

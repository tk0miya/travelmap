...
-- (`travelmap user create`) or the browser's own sign-up screen — both call
-- internal/auth.Register — and either way there is no column a sign-up flow
-- needs of its own. Likewise no status/plan/subscription_source/active_until:
-- those are upstream Cloud's billing fields, billing is a non-goal, and
-- auth/login answers them with constants instead.
--

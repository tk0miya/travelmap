...
// createSession answers POST /travelmap/web/session: an email and a password
// in, a session cookie out. It is the browser's own sign-in action, distinct
// from POST /api/v1/auth/login, which hands a Dawarich client its api_key
// instead.
//
...
// deleteSession answers DELETE /travelmap/web/session: the browser's own
// sign-out action, replacing the old POST /logout. Destroy deletes the
// session row, and LoadAndSave writes the cookie that clears it
// client-side — clearing the cookie alone, without deleting the row, would
// leave a session anyone holding the old token could still present.

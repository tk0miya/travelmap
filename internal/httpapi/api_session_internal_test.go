...
func postSession(t *testing.T, srv *httptest.Server, cookie string) *http.Response {
	...
	req, err := http.NewRequestWithContext(
		t.Context(), http.MethodPost, srv.URL+"/travelmap/web/session", strings.NewReader(body),
	)
	if err != nil {
	...
	if err != nil {
		t.Fatalf("POST /travelmap/web/session: %v", err)
	}

	return resp
}

// deleteSessionRequest is [postSession] for DELETE /travelmap/web/session,
// which takes no body.
func deleteSessionRequest(t *testing.T, srv *httptest.Server, cookie string) *http.Response {
	t.Helper()

	req, err := http.NewRequestWithContext(t.Context(), http.MethodDelete, srv.URL+"/travelmap/web/session", nil)
	if err != nil {
		t.Fatalf("building the request: %v", err)
	}
	...
	resp, err := srv.Client().Do(req)
	if err != nil {
		t.Fatalf("DELETE /travelmap/web/session: %v", err)
	}

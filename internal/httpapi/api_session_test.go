...
// sessionBody builds the JSON body POST /travelmap/web/session expects.
...
func TestCreateSession(t *testing.T) {
	...
	srv := newTestServer(t)
	resp := do(t, srv, http.MethodPost, "/travelmap/web/session", withBody(sessionBody(testEmail, testPassword)))
...
func TestCreateSessionRefused(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			srv := newTestServer(t)
			resp := do(t, srv, http.MethodPost, "/travelmap/web/session", withBody(sessionBody(tt.email, tt.password)))
...
func TestCreateSessionRejectsAnUnreadableBody(t *testing.T) {
	...
	srv := newTestServer(t)
	resp := do(t, srv, http.MethodPost, "/travelmap/web/session", withBody(`{"email":`))
...
func TestCreateSessionCrossSiteRejected(t *testing.T) {
	...
	srv := newTestServer(t)
	resp := do(t, srv, http.MethodPost, "/travelmap/web/session",
...
func TestDeleteSession(t *testing.T) {
	...
	srv := newTestServer(t)
	loginResp := do(t, srv, http.MethodPost, "/travelmap/web/session", withBody(sessionBody(testEmail, testPassword)))
	token := sessionCookie(t, loginResp)

	logoutResp := do(t, srv, http.MethodDelete, "/travelmap/web/session", withHeader("Cookie", "session="+token))

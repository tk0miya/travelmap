...
| User management | Issued via CLI (`travelmap user create`) or the browser's own sign-up screen at `/signup`, both through `auth.Register`. `auth/login` implemented, no 2FA | Self-hosted assumption. Sign-up is open to anyone — no environment variable, no invite code, no first-user-only rule — since a gate is one more setting to get wrong before the first login works, on a server whose first account is the operator's own |
